...
- Stale generated sources from code-generation plugins (Avro, OpenAPI, Protobuf, etc.) manifest as class redefinition errors (`duplicate class`) in otherwise correct repositories. Run `makevn test --clean-generated-contract-targets` or set `MAKEVN_CLEAN_GENERATED_CONTRACT_TARGETS=true` in `.makevn/config` to auto-clean stale contract outputs before every test/verify run. This is safe because `target/generated-sources/` is always regenerated by the generator plugin during compilation.
