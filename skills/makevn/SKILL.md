...
- Stale generated sources from code-generation plugins (Avro, OpenAPI, Protobuf, etc.) manifest as class redefinition errors (`duplicate class`) in otherwise correct repositories. **Proactively detect these cases**: after `makevn doctor`, scan `pom.xml` for `avro-maven-plugin`, `openapi-generator-maven-plugin`, `protobuf-maven-plugin`, or `maven-dependency-plugin` with an `unpack` goal. If any is present, add `--clean-generated-contract-targets` to your test/verify commands, or suggest setting `MAKEVN_CLEAN_GENERATED_CONTRACT_TARGETS=true` in `.makevn/config` for ongoing protection. The cleanup is safe because `target/generated-sources/` is always regenerated by the generator plugin during compilation.
...
7. **Proactively check for code-generation plugins**: scan `pom.xml` for `avro-maven-plugin`, `openapi-generator-maven-plugin`, `protobuf-maven-plugin`, or `maven-dependency-plugin` with an `unpack` goal. If any is found, use `--clean-generated-contract-targets` in test/verify commands, or suggest setting `MAKEVN_CLEAN_GENERATED_CONTRACT_TARGETS=true` in `.makevn/config`.
