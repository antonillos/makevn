...
case "${COMMAND}" in
  doctor|init|uninstall|profile|exec|compile|compile-tests|validate|package|build|clean|test|verify-ut|verify-ut-coverage|verify-it|verify-it-coverage|verify|verify-changes|coverage-changes|pr-verify|docker-up|docker-down|docker-ps|docker-ps-required|run|jdk)
    ;;
  *)
